...
const MakeKundali = ({ apiKey }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!apiKey) return alert(t('makeKundali.errors.apiKeyMissing'));
        if (!formData.name || !formData.dob || !formData.tob || !formData.pob) {
            return alert(t('makeKundali.errors.fillAllFields'));
        }
        ...
        try {
            ...
            console.error(error);
            alert(`${t('makeKundali.errors.unknown')}: ${error?.message || ''}`);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-4xl mx-auto animate-fade-in">
            <div className="text-center mb-8">
                ...
                <p className="text-purple-200">{t('makeKundali.subtitle')}</p>
            </div>

            <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 mb-6">
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <label className="block text-purple-100 mb-2 font-semibold">{t('makeKundali.fullName')}</label>
                        <input
                            type="text"
                            name="name"
                            value={formData.name}
                            onChange={handleChange}
                            placeholder={t('makeKundali.fullNamePlaceholder')}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-purple-100 mb-2 font-semibold">{t('makeKundali.dob')}</label>
                            <input
                                type="date"
                                name="dob"
                                value={formData.dob}
                        ...
                        <div>
                            <label className="block text-purple-100 mb-2 font-semibold">{t('makeKundali.tob')}</label>
                            <input
                                type="time"
                                name="tob"
                                value={formData.tob}
                    ...
                    <div>
                        <label className="block text-purple-100 mb-2 font-semibold">{t('makeKundali.pob')}</label>
                        <input
                            type="text"
                            name="pob"
                            value={formData.pob}
                            onChange={handleChange}
                            placeholder={t('makeKundali.pobPlaceholder')}
                            className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-purple-300 focus:outline-none focus:ring-2 focus:ring-purple-500"
                        />
                    </div>
                    ...
                    {formData.dob && (
                        <div className="bg-purple-500/20 rounded-xl p-4 border border-purple-500/30">
                            <p className="text-center text-purple-100">
                                <span className="font-semibold">{t('makeKundali.yourZodiacSign')}</span>{' '}
                    ...
                    >
                        {isLoading ? t('makeKundali.interpreting') : t('makeKundali.interpretKundali')}
                    </button>
                </form>
            </div>
            ...
            {isLoading && (
                <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 text-center">
                    ...
                    <p className="text-purple-200">{t('makeKundali.readingCosmic')}</p>
                </div>
            )}
            ...
            {result && (
                <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 animate-fade-in">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            <h3 className="text-2xl font-bold text-yellow-200">{t('makeKundali.resultTitle')}</h3>
                            <p className="text-purple-200 text-sm">{t('makeKundali.kundaliFor', { name: formData.name })}</p>
                        </div>
                    </div>
